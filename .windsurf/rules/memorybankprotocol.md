...
7. **Locking Instruction:** ALWAYS use absolute paths for memory-bank files in `C:\Users\kidpixel\Documents\kimi-proxy\memory-bank\`. Use EXCLUSIVELY the fast-filesystem and filesystem-agent MCP tools. Do NOT attempt to read memory-bank files via regular filesystem tools (read_file).
...
-   **Archiving Tool:** Use `fast_write_file(path="C:\Users\kidpixel\Documents\kimi-proxy\memory-bank\archives\...")` to archive old entries. Never delete data without archiving via this tool first.
