...
def map_model_name(
    ...
    # 2. Vérifier si le client envoie déjà le nom complet/upstream exact
    for m_data in models.values():
        if m_data.get("model") == client_model:
            logger.debug(f"Nom exact upstream trouvé: {client_model}")
            return client_model
