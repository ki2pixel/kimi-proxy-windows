...
def is_safe_ip(ip: ipaddress.IPv4Address | ipaddress.IPv6Address) -> bool:
    """Vérifie si une IP n'est pas privée, loopback ou réservée."""
    return not (
        ip.is_private or
        ip.is_loopback or
        ip.is_link_local or
...
async def validate_target_url(url: str, providers_config: Dict[str, Any]) -> bool:
    """Valide qu'une URL cible n'est pas vulnérable aux attaques SSRF."""
    try:
        parsed = urlparse(url)
    ...
    allowed_hosts = set()
    for provider_data in providers_config.values():
        if isinstance(provider_data, dict) and "base_url" in provider_data:
            try:
                p_url = urlparse(provider_data["base_url"])
                p_host = p_url.hostname
                if p_host:
                    allowed_hosts.add(p_host.lower())
            except Exception:
                continue

    host_lower = host.lower()
    is_allowed_domain = False
    for allowed_host in allowed_hosts.union(TRUSTED_DOMAINS):
        if host_lower == allowed_host or host_lower.endswith("." + allowed_host):
    ...
    ip_host = host
    if ip_host.startswith('[') and ip_host.endswith(']'):
        ip_host = ip_host[1:-1]
    ...
    try:
        ...
        try:
            import asyncio
            addr_info = await asyncio.to_thread(socket.getaddrinfo, ip_host, None)
...
async def resolve_target(
    ...
    """Resout la cible finale (base_url, provider_type, api_key).
    ...
    if target_base_url and isinstance(target_base_url, str):
        target_base_url_clean = target_base_url.strip()
        
        # Validation SSRF de la cible radicale
        if not await validate_target_url(target_base_url_clean, providers):
            raise HTTPException(
                status_code=400,
                detail="Blocked by SSRF protection: target base URL is not trusted or resolves to an unsafe IP address."
            )
        ...
        print(f"🎯 [PASSTHROUGH] Cible radicale: {target_base_url_clean} (type={provider_type})")
        return target_base_url_clean, provider_type.strip(), api_key
...
class PassthroughProcessor:
    ...
    async def forward(
        ...
        """
        base_url, provider_type, api_key = await resolve_target(
        ...
        if api_key:
            ...
            print("🔑 [PASSTHROUGH] Cle API injectee")
        else:
        ...
        try:
            if provider_type == "gemini":
                target_endpoint = build_gemini_endpoint(
                    base_url, str(body_json.get("model") or ""), api_key or "", is_streaming
                )
