...
from ...features.mcp import detect_and_store_memories
...
async def proxy_chat(request: Request, token: Optional[str] = Depends(verify_proxy_secret)):
    """
    ...
    # === Flux Passthrough Session-less (radicale/Cline) ===
    ...
    if target_base_url:
        ...
        processed_body = await processor.apply_features(json_body)
        return await processor.forward(processed_body, headers, request)
    ...
    if not session:
        session = {"id": 0, "provider": "managed:kimi-code", "model": None}
...
async def _proxy_to_provider(
    ...
    if provider_api_key:
        ...
        print(f"🔑 Clé API {provider_key} injectée")
    else:
