...
class QdrantMCPClient:
    ...
    async def check_status(self) -> "MCPExternalServerStatus":
        ...
        now = datetime.now()
        if (self._status_check_time and self._status is not None and
        ...
        is_cloud = "cloud.qdrant.io" in (self.config.qdrant_url or "")
