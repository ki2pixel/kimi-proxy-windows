...
class MCPClientConfig:
    ...
    # Task Master / Shrimp Task Manager MCP (Phase 4)
    # NOTE: les champs task_master_* sont conservés pour compatibilité tests/legacy.
    ...
    # Alias moderne (configuration interne)
    ...
    def __post_init__(self) -> None:
        """Synchronise les aliases task_master_* et shrimp_task_manager_*."""
        # URL
        if self.task_master_url and not self.shrimp_task_manager_url:
            self.shrimp_task_manager_url = self.task_master_url
        elif self.shrimp_task_manager_url and not self.task_master_url:
            self.task_master_url = self.shrimp_task_manager_url

        # API key
        if self.task_master_api_key and not self.shrimp_task_manager_api_key:
            self.shrimp_task_manager_api_key = self.task_master_api_key
        elif self.shrimp_task_manager_api_key and not self.task_master_api_key:
            self.task_master_api_key = self.shrimp_task_manager_api_key

        # Timeout
        if self.task_master_timeout_ms and not self.shrimp_task_manager_timeout_ms:
            self.shrimp_task_manager_timeout_ms = self.task_master_timeout_ms
        elif self.shrimp_task_manager_timeout_ms and not self.task_master_timeout_ms:
            self.task_master_timeout_ms = self.shrimp_task_manager_timeout_ms
    ...
    def from_toml(cls, toml_config: Dict[str, Any]) -> "MCPClientConfig":
        ...
        task_master_cfg = mcp.get("task_master", {})
        shrimp_task_master_cfg = mcp.get("shrimp_task_manager", {})

        task_master_url = task_master_cfg.get("url", shrimp_task_master_cfg.get("url", "http://localhost:8002"))
        task_master_api_key = task_master_cfg.get("api_key", shrimp_task_master_cfg.get("api_key"))
        task_master_timeout_ms = task_master_cfg.get("timeout_ms", shrimp_task_master_cfg.get("timeout_ms", 30000.0))
        ...
        return cls(
            ...
            # Task Master / Shrimp Task Manager
            task_master_url=task_master_url,
            task_master_api_key=task_master_api_key,
            task_master_timeout_ms=task_master_timeout_ms,
            shrimp_task_manager_url=task_master_url,
            shrimp_task_manager_api_key=task_master_api_key,
            shrimp_task_manager_timeout_ms=task_master_timeout_ms,
