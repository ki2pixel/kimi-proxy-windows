...
Entrée allowlistée (cross-platform):
    `${KIMI_PROXY_CLINE_LEDGER_PATH}` ou, par défaut,
...
def _resolve_default_ledger_path() -> str:
    """Résout le chemin ledger allowlisté de manière cross-platform."""
    override = Path(
        os.environ.get(
            "KIMI_PROXY_CLINE_LEDGER_PATH",
            str(Path.home() / ".cline" / "data" / "state" / "taskHistory.json"),
        )
    ).expanduser()
    return str(override)


ALLOWED_LEDGER_PATH = _resolve_default_ledger_path()
