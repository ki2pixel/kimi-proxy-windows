...
class AnalyticsSource:
    ...
    async def initialize(self) -> None:
        ...
        self.available = await asyncio.to_thread(os.path.exists, self.path)
        self.healthy = True
...
class ContinueLogSource(AnalyticsSource):
    ...
    async def poll(self) -> List[AnalyticsEvent]:
        ...
        self.available = True
        current_size = await asyncio.to_thread(os.path.getsize, self.path)
...
class KimiSessionSource(AnalyticsSource):
    ...
    async def initialize(self) -> None:
        ...
        await self._discover_all_existing_sessions()

    async def poll(self) -> List[AnalyticsEvent]:
        if not self._initialized:
            await self.initialize()

        if not await asyncio.to_thread(os.path.isdir, self.path):
            self.available = False
            return []

        self.available = True
        await self._refresh_roots()
        await self._discover_new_sessions()
    ...
    def _refresh_roots_sync(self) -> List[str]:
        try:
            return sorted(
    ...
    async def _refresh_roots(self) -> None:
        self._roots = await asyncio.to_thread(self._refresh_roots_sync)
        if self._roots:
            self._root_cursor %= len(self._roots)
        else:
            self._root_cursor = 0

    async def _discover_all_existing_sessions(self) -> None:
        for root_path in self._roots:
            session_dirs = await self._list_session_directories(root_path)
            for session_path in session_dirs:
                await self._register_session(session_path)

    async def _discover_new_sessions(self) -> None:
        if not self._roots:
            return
    ...
    def _list_session_directories_sync(self, root_path: str) -> List[str]:
        try:
            return sorted(
    ...
    async def _list_session_directories(self, root_path: str) -> List[str]:
        return await asyncio.to_thread(self._list_session_directories_sync, root_path)

    async def _register_session(self, session_path: str) -> None:
        if session_path in self._sessions:
            return

        context_path = os.path.join(session_path, "context.jsonl")
        metadata_path = os.path.join(session_path, "metadata.json")
        if not await asyncio.to_thread(os.path.exists, context_path):
            return

        initial_position = 0
        try:
            context_size = await asyncio.to_thread(os.path.getsize, context_path)
    ...
    async def _poll_session(self, state: KimiSessionState) -> List[AnalyticsEvent]:
        if not await asyncio.to_thread(os.path.exists, state.context_path):
            return []

        await self._refresh_metadata(state)

        current_size = await asyncio.to_thread(os.path.getsize, state.context_path)
        ...
        try:
            state.context_mtime = await asyncio.to_thread(os.path.getmtime, state.context_path)
        except OSError:
    ...
    async def _refresh_metadata(self, state: KimiSessionState) -> None:
        if not await asyncio.to_thread(os.path.exists, state.metadata_path):
            state.metadata = {}
            state.metadata_mtime = 0.0
            return

        try:
            metadata_mtime = await asyncio.to_thread(os.path.getmtime, state.metadata_path)
        except OSError:
            return
...
class LogWatcher:
    ...
    def __init__(
        ...
        self.broadcast_callback: Optional[Callable] = broadcast_callback
