...
def get_sanitizer_config(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    raw_tmp_dir = sanitizer_config.get("tmp_dir")
    tmp_dir: str
    if isinstance(raw_tmp_dir, str):
        stripped_tmp_dir = raw_tmp_dir.strip()
        # Si variable d'env non résolue (${VAR}) ou chaîne vide, fallback robuste.
