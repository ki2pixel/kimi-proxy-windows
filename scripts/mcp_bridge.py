...
def _resolve_windows_npm_global_command(command_name: str) -> str | None:
    if os.name != "nt":
        return None
    ...
    candidate = Path(appdata) / "npm" / command_name
...
def _build_ripgrep_agent_command() -> RelayCommand:
    windows_direct = _resolve_windows_npm_global_command("mcp-ripgrep.cmd")
    default_cmd = windows_direct if windows_direct is not None else ("npx.cmd" if os.name == "nt" else "npx")
    cmd = os.getenv("MCP_RIPGREP_COMMAND", default_cmd)
    if _is_npx_command(cmd):
        args = ["-y", "mcp-ripgrep"]
    else:
        args = []
...
def _stderr_log_spawn_failure(*, server_name: str, relay_cmd: RelayCommand, exc: Exception) -> None:
    """Log minimal et safe pour diagnostiquer un échec de spawn (stderr only)."""

    try:
        path_value = relay_cmd.env.get("PATH") if isinstance(relay_cmd.env, dict) else None
        path_preview = path_value[:240] if isinstance(path_value, str) else "<unset>"
        details = {
            "server": server_name,
            "event": "spawn_failed",
        ...
        sys.stderr.write("[mcp_bridge spawn error] " + json.dumps(details, ensure_ascii=False) + "\n")
        sys.stderr.flush()
    except Exception:
        return
...
async def _connect_stdin_reader() -> asyncio.StreamReader:
    ...
    # Windows/Python 3.13: connect_read_pipe peut déclencher des erreurs internes
    # (_ProactorReadPipeTransport._empty_waiter) selon le contexte de lancement.
    # On force donc un fallback thread-safe sur Windows.
    if os.name == "nt":
        def _stdin_feeder_windows() -> None:
            try:
                while True:
        ...
        feeder = threading.Thread(target=_stdin_feeder_windows, daemon=True)
        feeder.start()
        return reader
    ...
    try:
        ...
        # Fallback Windows/CI: certains contextes de pipes ne supportent pas
        # connect_read_pipe selon la policy/transport asyncio.
        # On alimente le StreamReader via un thread bloquant sur stdin.

        def _stdin_feeder() -> None:
            try:
                while True:
                    chunk = sys.stdin.buffer.readline()
                    if not chunk:
        ...
        feeder = threading.Thread(target=_stdin_feeder, daemon=True)
        feeder.start()
    return reader
...
async def _run_shrimp_task_manager_stdio_with_roots_shim() -> int:
    ...
    try:
        ...
        _stderr_log_spawn_failure(server_name="shrimp-task-manager", relay_cmd=relay_cmd, exc=e)
    ...
    if proc_wait_task in done and not stdin_task.done():
        ...
        if inflight_ids:
            _write_inflight_errors(
                ...
                message=(
                    "Le serveur MCP s'est arrêté avant de répondre. "
    ...
    if stdin_task in done and not proc_wait_task.done():
        stdin_exc = stdin_task.exception()
        if stdin_exc is not None:
            ...
            if inflight_ids:
                _write_inflight_errors(
                    ...
                    inflight_ids=inflight_ids,
                    message=(
                        "Le flux client->serveur a échoué avant réponse. "
                        + f"Détail: {stdin_exc}"
                    ),
                )
                inflight.clear()
...
async def _run_stdio_relay(server_name: str) -> int:
    ...
    try:
        ...
        _stderr_log_spawn_failure(server_name=server_name, relay_cmd=relay_cmd, exc=e)
