...
# Chemins runtime cross-platform (surchargeables par variables d'environnement)
OS_TMP_DIR="${KIMI_PROXY_OS_TMP_DIR:-${TMPDIR:-${TEMP:-${TMP:-${PWD}/.tmp}}}}"
TMP_BASE_DIR="${KIMI_PROXY_TMP_ROOT:-${OS_TMP_DIR}/kimi-proxy}"
...
FAST_FILESYSTEM_SERVER_SCRIPT="${TMP_RUNTIME_DIR}/mcp_fast_filesystem_server.py"
JSON_QUERY_SERVER_SCRIPT="${TMP_RUNTIME_DIR}/mcp_json_query_server.py"

COMPRESSION_LOG_FILE="${TMP_LOG_DIR}/mcp_compression.log"
SEQUENTIAL_THINKING_LOG_FILE="${TMP_LOG_DIR}/mcp_sequential_thinking.log"
FAST_FILESYSTEM_LOG_FILE="${TMP_LOG_DIR}/mcp_fast_filesystem.log"
JSON_QUERY_LOG_FILE="${TMP_LOG_DIR}/mcp_json_query.log"
PRUNER_LOG_FILE="${TMP_LOG_DIR}/mcp_pruner.log"

COMPRESSION_PID_FILE="${TMP_RUNTIME_DIR}/mcp_compression.pid"
SEQUENTIAL_THINKING_PID_FILE="${TMP_RUNTIME_DIR}/mcp_sequential_thinking.pid"
FAST_FILESYSTEM_PID_FILE="${TMP_RUNTIME_DIR}/mcp_fast_filesystem.pid"
JSON_QUERY_PID_FILE="${TMP_RUNTIME_DIR}/mcp_json_query.pid"
PRUNER_PID_FILE="${TMP_RUNTIME_DIR}/mcp_pruner.pid"
...
start_servers() {
    ...
    mkdir -p "$TMP_RUNTIME_DIR" "$TMP_LOG_DIR"
    ...
    if check_port $COMPRESSION_PORT; then
        ...
        WORKSPACE_PATH="$(pwd)" nohup python3 "$COMPRESSION_SERVER_SCRIPT" > "$COMPRESSION_LOG_FILE" 2>&1 &
        COMPRESSION_PID=$!
        ...
        if check_port $COMPRESSION_PORT; then
            ...
            echo $COMPRESSION_PID > "$COMPRESSION_PID_FILE"
        else
            log_error "Échec du démarrage du serveur de compression"
            echo "   Logs: $COMPRESSION_LOG_FILE"
            exit 1
        fi
    fi
    ...
    if check_port $SEQUENTIAL_THINKING_PORT; then
        ...
        # Démarrer en arrière-plan avec nohup
        WORKSPACE_PATH="$(pwd)" nohup python3 "$SEQUENTIAL_THINKING_SERVER_SCRIPT" > "$SEQUENTIAL_THINKING_LOG_FILE" 2>&1 &
        ...
        if check_port $SEQUENTIAL_THINKING_PORT; then
            ...
            echo $SEQUENTIAL_THINKING_PID > "$SEQUENTIAL_THINKING_PID_FILE"
        else
            log_error "Échec du démarrage du serveur de sequential thinking"
            echo "   Logs: $SEQUENTIAL_THINKING_LOG_FILE"
            exit 1
        fi
    fi
    ...
    if check_port $FAST_FILESYSTEM_PORT; then
        ...
        if check_port $FAST_FILESYSTEM_PORT; then
            ...
            echo $FAST_FILESYSTEM_PID > "$FAST_FILESYSTEM_PID_FILE"
        else
            log_error "Échec du démarrage du serveur de fast filesystem"
            echo "   Logs: $FAST_FILESYSTEM_LOG_FILE"
            exit 1
        fi
    fi
    ...
    if check_port $JSON_QUERY_PORT; then
        ...
        if check_port $JSON_QUERY_PORT; then
            ...
            echo $JSON_QUERY_PID > "$JSON_QUERY_PID_FILE"
        else
            log_error "Échec du démarrage du serveur de json query"
            echo "   Logs: $JSON_QUERY_LOG_FILE"
            exit 1
        fi
    fi
...
start_pruner_server() {
    ...
    while [ $waited_s -lt $max_wait_s ]; do
        if check_port $PRUNER_PORT; then
            ...
            echo $PRUNER_PID > "$PRUNER_PID_FILE"
            return
        fi
    ...
    echo "   Logs: $PRUNER_LOG_FILE"
    echo "   PID: $PRUNER_PID"
    if [ -f "$PRUNER_LOG_FILE" ]; then
        echo ""
        echo "--- Dernières lignes ($PRUNER_LOG_FILE) ---"
        tail -n 80 "$PRUNER_LOG_FILE" 2>/dev/null || true
    fi
    exit 1
}
...
create_compression_server() {
    cat > "$COMPRESSION_SERVER_SCRIPT" << 'EOF'
#!/usr/bin/env python3
"""
...
WORKSPACE_PATH = os.getenv("WORKSPACE_PATH", str(Path.home()))
...
if __name__ == "__main__":
    run_server()
EOF
    chmod +x "$COMPRESSION_SERVER_SCRIPT"
}
...
create_sequential_thinking_server() {
    cat > "$SEQUENTIAL_THINKING_SERVER_SCRIPT" << 'EOF'
#!/usr/bin/env python3
"""
...
if __name__ == "__main__":
    run_server()
EOF
    chmod +x "$SEQUENTIAL_THINKING_SERVER_SCRIPT"
}
...
create_fast_filesystem_server() {
    cat > "$FAST_FILESYSTEM_SERVER_SCRIPT" << 'EOF'
#!/usr/bin/env python3
"""
...
if __name__ == "__main__":
    run_server()
EOF
    chmod +x "$FAST_FILESYSTEM_SERVER_SCRIPT"
}
...
create_json_query_server() {
    cat > "$JSON_QUERY_SERVER_SCRIPT" << 'EOF'
#!/usr/bin/env python3
"""
...
if __name__ == "__main__":
    run_server()
EOF
    chmod +x "$JSON_QUERY_SERVER_SCRIPT"
}
...
stop_servers() {
    ...
    if [ -f "$COMPRESSION_PID_FILE" ]; then
        pid=$(cat "$COMPRESSION_PID_FILE")
        if kill -0 $pid 2>/dev/null; then
            kill -9 $pid 2>/dev/null || true
            log_success "Compression MCP arrêté (PID: $pid)"
        fi
        rm -f "$COMPRESSION_PID_FILE"
    fi

    # Arrêter le serveur de sequential thinking
    if [ -f "$SEQUENTIAL_THINKING_PID_FILE" ]; then
        pid=$(cat "$SEQUENTIAL_THINKING_PID_FILE")
        if kill -0 $pid 2>/dev/null; then
            kill -9 $pid 2>/dev/null || true
            log_success "Sequential Thinking MCP arrêté (PID: $pid)"
        fi
        rm -f "$SEQUENTIAL_THINKING_PID_FILE"
    fi
    
    # Arrêter le serveur de fast filesystem
    if [ -f "$FAST_FILESYSTEM_PID_FILE" ]; then
        pid=$(cat "$FAST_FILESYSTEM_PID_FILE")
        if kill -0 $pid 2>/dev/null; then
            kill -9 $pid 2>/dev/null || true
            log_success "Fast Filesystem MCP arrêté (PID: $pid)"
        fi
        rm -f "$FAST_FILESYSTEM_PID_FILE"
    fi
    
    # Arrêter le serveur de json query
    if [ -f "$JSON_QUERY_PID_FILE" ]; then
        pid=$(cat "$JSON_QUERY_PID_FILE")
        if kill -0 $pid 2>/dev/null; then
            kill -9 $pid 2>/dev/null || true
            log_success "JSON Query MCP arrêté (PID: $pid)"
        fi
        rm -f "$JSON_QUERY_PID_FILE"
    fi

    # Arrêter le serveur MCP Pruner
    if [ -f "$PRUNER_PID_FILE" ]; then
        pid=$(cat "$PRUNER_PID_FILE")
        if kill -0 $pid 2>/dev/null; then
            kill -9 $pid 2>/dev/null || true
            log_success "MCP Pruner arrêté (PID: $pid)"
        fi
        rm -f "$PRUNER_PID_FILE"
    fi
