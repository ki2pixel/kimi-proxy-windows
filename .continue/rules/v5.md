...
### 2.2 Memory Bank (fast-filesystem & filesystem-agent MCP)
- **Selective Pulling**: N'utilisez JAMAIS de lecture globale du dossier `memory-bank\`. Pratiquez la lecture chirurgicale.
- **Reading**: Utilisez `fast_read_file(path="/absolute/path").
- **Updating**: Utilisez `edit_file(path="\path\to\file", old_text="...", new_text="...")` pour synchroniser l'état du projet en fin de tâche.
- **Writing**: Utilisez `fast_write_file(path="\path\to\file", content="...")` uniquement pour l'archivage ou l'initialisation des 5 fichiers de base.
- **Listing**: Utilisez `fast_list_directory(path="C:\Users\kidpixel\Documents\kimi-proxy\memory-bank")` pour lister les fichiers disponibles.
- **Project Listing**: Utilisez `fast_get_directory_tree(path="C:\Users\kidpixel\Documents\kimi-proxy\memory-bank")` pour diagnostiquer la connectivité en cas d'échec d'accès.
...
- **Authorized Tools**: `edit_file` (via filesystem-agent), `fast_read_multiple_files` (via fast-filesystem), outils filesystem-agent et ripgrep-agent (MCP intégrés Continue).
- **Note importante**: Les outils MCP Phase 4 (shrimp-task-manager, sequential-thinking, fast-filesystem, json-query) sont maintenant disponibles directement dans Continue.dev via le bridge.
...
- **JSON Data Handling**: Pour les fichiers JSON volumineux, interdiction de les lire entièrement (`read_file`). Utilise `json_query` (outil MCP JSON Query **local uniquement**, pas disponible dans Continue) ou l'API proxy.
