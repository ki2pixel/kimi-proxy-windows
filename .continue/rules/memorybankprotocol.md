...
**Actions Required:**
1. **Start with MCP Pull**: Call `fast_read_file(path="C:\Users\kidpixel\Documents\kimi-proxy\memory-bank\activeContext.md")`.
...
7. **Locking Instruction**: Use absolute paths exclusively for memory-bank files in `C:\Users\kidpixel\Documents\kimi-proxy\memory-bank\`. Use EXCLUSIVELY the fast-filesystem and filesystem-agent MCP tools. Do NOT attempt to read memory-bank files via regular filesystem tools (read_file).

---

## 2. File Structure & Responsibilities
Access these via `fast_read_file`, `edit_file`, or `fast_list_directory` using absolute paths in `C:\Users\kidpixel\Documents\kimi-proxy\memory-bank\`.
...
-  - **Archiving Tool**: Use `fast_write_file(path="C:\Users\kidpixel\Documents\kimi-proxy\memory-bank\archives\...")` for archiving old entries. Never delete data without archiving via this tool first.
-  - **Creation Restriction**: `fast_write_file` only for initialization or archiving of the 5 base files) or archiving. Interdiction de créer de nouveaux types de fichiers à la racine de `memory-bank\` sans autorisation.
...
-   **Selective Pull:** Immediately call `read_file` for `.continue\rules\codingstandards.md`.
